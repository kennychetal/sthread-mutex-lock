...
static struct sthread_struct *
threadlist_findbypid(int pid)
{
...
static int
threadlist_add(struct sthread_struct *t)
{
  threadlist_lock();
  t->next = list_hd;
  list_hd = t;
...
static int
threadlist_del(struct sthread_struct *t)
{
...
static void
_sthread_self_suspend(struct sthread_struct *self)
{
  char buf[9];
  if (self == NULL) {
...
static void
thread_dealloc(struct sthread_struct *t)
{
  threadlist_del(t);
...
static void
do_create(struct sthread_struct *caller,
...
int
sthread_create(sthread_t *t, sthread_main_t main_routine, void *arg)
{
  manager_msg_t msg;
  struct sthread_struct *self = sthread_self();
...
int
test_and_set(volatile unsigned long *x)
{
  return tas(x);
}
